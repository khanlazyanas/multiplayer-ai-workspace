...
export const FloatingBubbleMenu = ({ editor }: { editor: any }) => {
  ...
  const handleAIAssist = async (action: string) => {
    ...
    if (action === 'explain') {
      ...
      promptTemplate = `Fix all grammar, punctuation, and spelling mistakes in the following text. Only return the corrected text:\n\n${selectedText}`;
      aiTitle = "✅ Grammar Fix";
    }

    setIsLoading(action); 
    const toastId = toast.loading(`AI is working on it...`, {
      ...
      {isCodeMode ? (
        ...
        <>
          <button 
            ...
            disabled={isLoading !== null} 
            className="px-3 py-1.5 text-xs font-semibold text-sky-400 hover:bg-sky-500/20 hover:text-sky-300 rounded-lg transition-all flex items-center gap-2 disabled:opacity-50"
          >
            {isLoading === 'improve' ? <SpinnerIcon /> : <PenIcon />}
            Improve
          </button>
          <button 
            onClick={() => handleAIAssist('grammar')} 
            disabled={isLoading !== null} 
            className="px-3 py-1.5 text-xs font-semibold text-amber-400 hover:bg-amber-500/20 hover:text-amber-300 rounded-lg transition-all flex items-center gap-2 disabled:opacity-50"
          >
            {isLoading === 'grammar' ? <SpinnerIcon /> : <CheckIcon />}
            Fix Grammar
          </button>
        </>
      )}

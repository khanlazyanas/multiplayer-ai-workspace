  ...
  return (
    ...
    >
      ...
      <main className="flex-1 relative w-full z-10 overflow-hidden bg-black">
        {/* 🔥 FIX: Dono modes ko hamesha mounted rakha hai. Opacity aur pointer-events se switch ho rahe hain taaki Canvas unmount na ho! */}
        
        {/* Editor Layer */}
        <div className={`absolute inset-0 w-full h-full overflow-y-auto py-10 px-4 md:px-0 flex justify-center z-10 transition-opacity ${activeMode === 'document' ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        {/* Canvas Layer */}
        <div className={`absolute inset-0 w-full h-full bg-[#111111] z-10 transition-opacity ${activeMode === 'canvas' ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
          <Canvas />
        </div>
      </main>
    </div>
